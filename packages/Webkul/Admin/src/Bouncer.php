...
class Bouncer
{
    ...
    public function getAuthorizedUserIds()
    {
        ...
        elseif ($user->view_permission == 'department') {
            $profile = \Company\Core\User\Models\EmployeeProfile::where('user_id', $user->id)->first();
            if ($profile && $profile->department_id) {
                return \Company\Core\User\Models\EmployeeProfile::where('department_id', $profile->department_id)->pluck('user_id')->toArray();
            }
            return [$user->id];
        }
        else {
